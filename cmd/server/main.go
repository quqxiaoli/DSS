...
var globalClient = &http.Client{
	...
	Timeout: 15 * time.Second, // 整体超时设为8秒，给足够响应时间但不过长
}
...
func forwardRequest(method, url, apiKey, key, value string) ([]Response, error) {
	...
	for i := 0; i < maxRetries; i++ {
		...
		url = strings.TrimPrefix(url, "https://") // 规范化URL
		url = "https://" + url
...
func main() {
	...
	http.HandleFunc("/set", func(w http.ResponseWriter, r *http.Request) {
		...
		localAddrNormalized = strings.TrimPrefix(localAddrNormalized, "http://")
		w.Header().Set("Content-Type", "application/json")
		if key == "" || value == "" {
			w.WriteHeader(http.StatusBadRequest)
			json.NewEncoder(w).Encode(Response{Error: "Key or value cannot be empty"})
		...
		// 添加详细调试日志
		logWithRateLimit(infoLogger, "Set request: key=%s, node=%s, localAddr=%s, X-Forwarded=%s", key, node, localAddrNormalized, r.Header.Get("X-Forwarded"))
